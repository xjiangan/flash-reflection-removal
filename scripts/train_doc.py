...
path_arr=df.to_numpy()
print(path_arr.shape)
num_img=len(path_arr)
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    shadow_mask_layer = tf.math.sigmoid(
                        UNet_SE(tf.concat([img_with_shadow, gray_pureflash], axis=3), output_channel = 3, ext='Ref_')
                        )
...
for epoch in range(92,maxepoch):
