import numpy as np
from utils.io import extract_exif
import os
...
def rgbg2srgb(raw_array,gamma=2.2,maximum=65535):
    return (lrgb2srgb(linref2lrgb(rgbg2linref(raw_array,maximum)))*225).astype(np.uint8)

# docshadow fm: [[ 0.51215854,0.26953794,0.17924102],
# [ 0.10933821,0.85002241,0.04063938],
# [ 0.01164852 ,-0.43424423,1.24290822]]
# nwb: [0.59085778, 1. ,0.44278711]
def linref2lrgb(linref,forward_matrix=np.array([[ 0.42787651 , 0.41633501 , 0.11672597],
                                [-0.2214807  , 1.26785571, -0.04637501],
                                [ 0.00282099, -0.39606465 , 1.21355616]]),
                           neutral_wb=np.array([0.43741756 ,1.       ,  0.28915222])):
